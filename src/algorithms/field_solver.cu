...
kernel_rk_push(const Scalar *ex, const Scalar *ey, const Scalar *ez,
  ...
  for (int k = threadIdx.z + blockIdx.z * blockDim.z + dev_grid.guard[2];
    ...
    for (int j = threadIdx.y + blockIdx.y * blockDim.y + dev_grid.guard[1];
        j < dev_grid.dims[1] - dev_grid.guard[1];
        j += blockDim.y * gridDim.y) {
      for (int i = threadIdx.x + blockIdx.x * blockDim.x + dev_grid.guard[0];
        ...
        dex[ijk] = CCx * ((by[ijkM1] - by[ijk] - bz[ijM1k] + bz[ijk]) -
                          (b0y[ijkM1] - bz0[ijk] - bz0[ijM1k] + bz0[ijk]));
...
__global__ void
kernel_rk_update(Scalar *ex, Scalar *ey, Scalar *ez,
...
void field_solver::copy_fields() {
  En.copy_form(m_data.E);
  Bn.copy_form(m_data.B);
}

void field_solver::rk_push() {
  kernel_rk_push<<<..., ...>>>(m_data.E.dev_ptr(0), m_data.E.dev_ptr(1), m_data.E.dev_ptr(2),
...
void field_solver::rk_update(Scalar rk_c1, Scalar rk_c2, Scalar rk_c3) {
  kernel_rk_update<<<..., ...>>>(m_data.E.dev_ptr(0), m_data.E.dev_ptr(1), m_data.E.dev_ptr(2),
                                 m_data.B.dev_ptr(0), m_data.B.dev_ptr(1), m_data.B.dev_ptr(2),
                                 En.dev_ptr(0), En.dev_ptr(1), En.dev_ptr(2),
                                 Bn.dev_ptr(0), Bn.dev_ptr(1), Bn.dev_ptr(2),
                                 dE.dev_ptr(0), dE.dev_ptr(1), dE.dev_ptr(2),
                                 dB.dev_ptr(0), dB.dev_ptr(1), dB.dev_ptr(2),
                                 rk_c1, rk_c2, rk_c3);
}

void field_solver::clean_epar() {
  kernel_clean_epar<<<..., ...>>>();
}
