...
sim_environment::exchange_type(MPI_Datatype* y_type,
  ...
  MPI_Type_vector(m_grid.dims[2], m_grid.dims[0],
                  m_grid.dims[0] * m_grid.dims[1], MPI_SCALAR, y_type);
  MPI_Type_commit(y_type);

  // Data exchange along x direction
  MPI_Type_vector(m_grid.dims[1], 1, m_grid.dims[0], MPI_SCALAR,
                  &x_temp);
  MPI_Type_commit(&x_temp);
  MPI_Type_vector(m_grid.dims[2], 1, m_grid.dims[0], x_temp, x_type);
...
sim_environment::send_guard_cell_x(sim_data& data, int dir) {
  ...
  // MPI_Send(data.E.dev_ptr(2) + send_offset, m_grid.guard[0], x_type,
  //          dest, 2, m_cart);
  // MPI_Recv(data.E.dev_ptr(2) + receive_offset, m_grid.guard[0], x_type,
  //          origin, 2, m_cart, &status);

  // MPI_Send(data.B.dev_ptr(0) + send_offset, m_grid.guard[0], x_type,
  //          dest, 3, m_cart);
  // MPI_Recv(data.B.dev_ptr(0) + receive_offset, m_grid.guard[0], x_type,
  //          origin, 3, m_cart, &status);
  // MPI_Send(data.B.dev_ptr(1) + send_offset, m_grid.guard[0], x_type,
  //          dest, 4, m_cart);
  // MPI_Recv(data.B.dev_ptr(1) + receive_offset, m_grid.guard[0], x_type,
  //          origin, 4, m_cart, &status);
  // MPI_Send(data.B.dev_ptr(2) + send_offset, m_grid.guard[0], x_type,
  //          dest, 5, m_cart);
  // MPI_Recv(data.B.dev_ptr(2) + receive_offset, m_grid.guard[0], x_type,
