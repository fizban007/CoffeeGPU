...
parse_config(const std::string& filename) {
  ...
  result.KOeps =
      config->get_as<double>("KOeps").value_or(defaults.KOeps);
  ...
  result.q_offset_x = config->get_as<double>("q_offset_x")
                          .value_or(defaults.q_offset_x);
  result.q_offset_y = config->get_as<double>("q_offset_y")
                          .value_or(defaults.q_offset_y);
  result.q_offset_z = config->get_as<double>("q_offset_z")
                          .value_or(defaults.q_offset_z);

  result.tp_start =
      config->get_as<double>("tp_start").value_or(defaults.tp_start);
  result.tp_end =
      config->get_as<double>("tp_end").value_or(defaults.tp_end);
  result.rpert1 =
      config->get_as<double>("rpert1").value_or(defaults.rpert1);
  result.rpert2 =
  ...
  result.rin = config->get_as<double>("rin").value_or(defaults.rin);
